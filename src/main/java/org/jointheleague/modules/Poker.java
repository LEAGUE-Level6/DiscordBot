...
public class Poker extends CustomMessageCreateListener {
	...
	String[] botAndMiddleCards = new String[8];
	int[] usedCards = new int[8];
	int playerCard1;
	int playerCard2;
	int middleCard1;
	int middleCard2;
		...
		if (event.getMessageContent().contains(command)) {
			...
			if (content.contains("poker")) {
				...
				if (balance < 0) {
					...
					event.getChannel()
							.sendMessage("Your cards are the " + cards[playerCard1] + " and the " + cards[playerCard2]);
			...
			if (botCallChance >= i) {
				// bot calls
				event.getChannel()
						.sendMessage("Bot calls. Bot chance of calling was " + botCallChance + ". The random was " + i);
				...
				event.getChannel().sendMessage("The middle cards are now the " + cards[middleCard1] + ", the "
						+ cards[middleCard2] + ", the " + cards[middleCard3]+" and the "+cards[middleCard4]);
				...
				event.getChannel().sendMessage("The bot has " + botAndMiddleCards[0] + botAndMiddleCards[1]
						+ botAndMiddleCards[2] + botAndMiddleCards[3] + botAndMiddleCards[4]+botAndMiddleCards[5]);
			} else {
				balance += totalBet*2;
				event.getChannel()
						.sendMessage("The bot folded, you win " + totalBet*2 + ". Your balance is now " + balance);
			}
			// in the future maybe add a choice to add players
		}
			}
			else if(content.contains("fold")) {
				event.getChannel().sendMessage("You folded. Your balance is "+balance);
			}
		}
	}
