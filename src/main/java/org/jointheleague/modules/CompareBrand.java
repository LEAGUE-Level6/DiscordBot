...
public class CompareBrand extends CustomMessageCreateListener {
	Car car = new Car("Acura ILX", 5, 201, 26100);
	Car[] scars = {new Car("Acura ILX", 5, 201, 26100), new Car("Acura MDX", 7, 2090, 46900), new Car("Acura TLX", 5, 272, 37500), new Car("Acura RDX", 5, 272, 38400), 
	...
	public void handle(MessageCreateEvent event) throws APIException {
		...
		for (int i = 0; i < cars.length; i++) {
			if (hasBudget && hasSeats && hasHorsies) {
				budgetMet = car.checkPrice(Integer.parseInt(car[i][3]), budget);
				hpMet = car.checkPrice(Integer.parseInt(car[i][2]), horsepower);
				seatsMet = car.checkPrice(Integer.parseInt(car[i][1]), seats);
				if (budgetMet && hpMet && seatsMet) {
...
class Car {
	...
	public boolean checkHp(int hp, int hpLimit) {
		if (hp <= hpLimit) {
			return true;
		}
		return false;
	...
	public boolean checkSeats(int seats, int seatLimit) {
		if (seats <= seatLimit) {
			return true;
		}
		return false;
	...
	public boolean checkPrice(int price, int priceLimit) {
		if (hp <= priceLimit) {
			return true;
		}
		return false;
